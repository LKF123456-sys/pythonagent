...
const WS_API_PREFIX = "/api/v1"; // 后端工业 WebSocket 路由也被统一挂载到 API v1 前缀下
...
export class MfgSocket {
  ...
  connect(): void {
    ...
    const url = `${proto}://${window.location.host}${WS_API_PREFIX}/ws/manufacturing/${this.sessionId}?token=${encodeURIComponent(token)}`;
