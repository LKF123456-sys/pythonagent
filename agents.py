"""
智能体定义模块：调度主管、搜索 Agent、视觉 Agent、回答 Agent。
每个智能体职责单一，不持有超出自身职责的工具。
"""

# 导入 base64 模块，用于图片 base64 编码
import base64
# 导入 os 模块，用于文件路径操作
import os
# 导入类型提示模块
...
# 从 langchain_openai 导入 ChatOpenAI，用于调用 OpenAI 兼容的 LLM 接口
from langchain_openai import ChatOpenAI
# 从 langchain_core.messages 导入 HumanMessage 和 SystemMessage，用于构建消息
from langchain_core.messages import HumanMessage, SystemMessage
# 从 tavily 导入 TavilyClient，用于联网搜索
from tavily import TavilyClient
# 导入 ollama 模块，用于调用本地 Ollama 模型
import ollama
...
def _create_llm(temperature: float = 0.0) -> ChatOpenAI:
    ...
    # 返回配置好的 ChatOpenAI 实例，指向 DeepSeek API
    return ChatOpenAI(
        model=Config.MODEL_NAME,               # 模型名称
        api_key=Config.OPENAI_API_KEY,         # API 密钥
        base_url=Config.OPENAI_BASE_URL,       # API 基础 URL
...
def supervisor_decide(user_question: str, history_context: str = "") -> str:
    """
    调度主管 Agent：分析用户问题，结合对话历史判断处理方式。
    ...
    # 获取 LLM 返回内容，去除首尾空白并转换为大写
    result_text = response.content.strip().upper()
    # 通过文本包含判断路由结果（DeepSeek 不支持结构化输出，用简单字符串匹配）
...
# ============================================================
# 2. 搜索 Agent（唯一持有联网搜索能力）
# ============================================================

# 搜索 Agent 系统提示词：只输出搜索关键词
...
def search_web(user_question: str) -> str:
    """
    搜索 Agent：使用 Tavily 执行联网搜索，返回结构化摘要。
    """
    # 创建温度为 0 的 LLM 实例
    llm = _create_llm(temperature=0.0)
    # 调用 LLM 提取搜索关键词
...
# ============================================================
# 3. 视觉 Agent（使用本地 Ollama qwen3 多模态识别图片）
# ============================================================

# 视觉 Agent 系统提示词：定义视觉分析的要求
...
def analyze_image(image_path: str, user_question: str = "") -> str:
    """
    视觉 Agent：使用本地 Ollama 多模态模型识别图片内容。
    ...
    try:
        response = ollama.chat(
            ...
            messages=[{
                "role": "user",
                "content": prompt,
                "images": [image_data],       # 传入 base64 编码的图片
            }],
        )
        # 记录图片分析完成日志
        ...
        # 如果 Ollama 调用失败，记录错误日志并返回错误提示
        logger.error("视觉 Agent 失败：%s", e)
        return f"[视觉识别失败] 请确保 Ollama 服务已启动且已安装{Config.OLLAMA_VISION_MODEL}模型。错误：{str(e)}"


# ============================================================
# 4. 回答 Agent（综合所有上下文生成最终答案）
# ============================================================

# 回答 Agent 系统提示词：要求按指定格式输出思考过程和最终回答
...
def generate_answer(
    ...
    """
    回答 Agent：综合所有上下文生成最终回答（非流式）。

    Args:
        user_question: 用户原始问题
        search_results: 联网搜索结果
        rag_context: RAG 文档检索结果
...
async def generate_answer_stream(
    ...
    Yields:
        str: 逐 token 的回答文本
    """
    # 导入 AsyncGenerator 类型（延迟导入避免循环引用）
    ...
    # 使用流式 API 调用 LLM，逐 token 产出
    async for chunk in llm.astream([
        SystemMessage(content=ANSWER_SYSTEM_PROMPT),
        HumanMessage(content=user_message),
    ]):
        # 如果 chunk 有 content 属性，yield 该内容
