...
async def _run_agent_stream_impl(  # 定义流式执行的实际实现函数
    ...
    final_answer = ""  # 回答节点最终消息兜底文本，避免模型流事件缺失时done.answer为空
    ...
    def _feed_final_text(text: str) -> list:  # 定义将最终文本重新喂给标签解析器的内部函数
        """解析最终文本并补发前端事件。"""
        emitted = []  # 创建需要补发给前端的事件列表
        if not text:  # 如果最终文本为空
            return emitted  # 直接返回空事件列表
        for ev in parser.feed(text):  # 将最终文本喂给标签解析器，提取thinking和answer正文
            if ev.type == "thinking":  # 如果解析出思考内容
                emitted.append(GraphStreamEvent(type="thinking", content=ev.content))  # 转换为thinking事件
            elif ev.content:  # 如果解析出回答正文
                answer_parts.append(ev.content)  # 累积回答正文
                emitted.append(GraphStreamEvent(type="token", content=ev.content))  # 转换为token事件
        emitted.extend(_drain_parser())  # 刷出解析器尾部剩余内容
        return emitted  # 返回补发事件列表
    ...
    try:  # 开始异常捕获块
        async for event in graph.astream_events(  # 异步迭代图的事件流
            ...
            # 捕获 answer 节点最终输出，用作模型流事件缺失时的答案兜底
            if event_type == "on_chain_end" and event_name == NODE_ANSWER:  # 如果是回答节点结束事件
                extracted_answer = _extract_final_answer(event.get("data", {}).get("output"))  # 从节点输出中提取最终回答文本
                if extracted_answer:  # 如果提取到了非空回答
                    final_answer = extracted_answer  # 保存为最终回答兜底文本
                continue  # 跳过后续处理
        ...
        answer_text = "".join(answer_parts)  # 优先使用流式token累积结果
        if not answer_text and final_answer:  # 如果流式token缺失但回答节点结束事件中有最终文本
            for ev in _feed_final_text(final_answer):  # 将最终文本重新解析为thinking/token事件
                yield ev  # 补发解析后的事件给前端
            answer_text = "".join(answer_parts) or final_answer  # 重新计算解析后的最终回答文本
        if not answer_text and hasattr(graph, "aget_state"):  # 如果事件流没有捕获答案且图支持读取检查点状态
            snapshot = await graph.aget_state(config)  # 按当前thread_id读取工作流执行结束后的最终状态快照
            snapshot_values = getattr(snapshot, "values", {}) or {}  # 获取状态快照中的最终状态字典
            checkpoint_answer = _extract_final_answer(snapshot_values)  # 从最终状态消息列表中提取真实AI回答
            if checkpoint_answer:  # 如果从检查点读取到了真实回答
                logger.info("流式事件未捕获答案，已从最终检查点状态恢复回答（%d字符）", len(checkpoint_answer))  # 记录恢复成功日志
                for ev in _feed_final_text(checkpoint_answer):  # 将检查点回答重新解析为thinking/token事件
                    yield ev  # 补发解析后的事件给前端
                answer_text = "".join(answer_parts) or checkpoint_answer  # 重新计算最终回答文本
            else:  # 如果检查点中也没有恢复到回答
                logger.warning("流式事件未捕获答案，最终检查点状态也没有可用AI回答，状态字段=%s", list(snapshot_values.keys()))  # 记录诊断日志
        if not answer_text:  # 如果事件流和最终检查点状态都没有可显示回答
            answer_text = "抱歉，这一轮智能体流程已经结束，但没有生成可显示的回答。请稍后重试，或换一种问法再试一次。"  # 构造前端可见的兜底回复
        yield GraphStreamEvent(  # 产出完成事件
            type="done",  # 事件类型为done
            answer=answer_text,  # 设置最终回答文本
...
def _extract_final_answer(output) -> str:  # 定义从回答节点输出中提取最终回答文本的私有函数
    """从 answer 节点输出中提取最后一条 AIMessage 文本。"""
    if output is None:  # 如果输出为空
        return ""  # 返回空字符串
    if hasattr(output, "content"):  # 如果输出本身就是消息对象
        return _message_content_text(output)  # 直接提取消息文本
    if isinstance(output, list):  # 如果输出是列表
        for item in reversed(output):  # 从后往前遍历列表
            text = _extract_final_answer(item)  # 递归提取当前元素中的回答文本
            if text:  # 如果当前元素提取到文本
                return text  # 返回该文本
        return ""  # 列表中没有可用文本则返回空字符串
    if not isinstance(output, dict):  # 如果输出不是字典
        return ""  # 返回空字符串
    messages = output.get("messages") or []  # 从输出字典中获取消息列表，缺失则使用空列表
    for message in reversed(messages):  # 从后往前遍历消息，优先取最新的助手消息
        text = _message_content_text(message)  # 提取消息文本
        if text:  # 如果提取到非空文本
            return text  # 返回最终回答文本
    for key in ("output", "__end__", "state", "result"):  # 遍历LangGraph常见的嵌套输出字段
        text = _extract_final_answer(output.get(key))  # 递归提取嵌套字段中的回答文本
        if text:  # 如果嵌套字段中提取到文本
            return text  # 返回嵌套回答文本
    return ""  # 未找到可用回答则返回空字符串


def _message_content_text(message) -> str:  # 定义消息内容转文本的私有函数
    if isinstance(message, dict):  # 如果消息是LangGraph检查点序列化后的字典格式
        message_type = str(message.get("type") or message.get("role") or "").lower()  # 读取消息类型或角色并统一转为小写
        if message_type and message_type not in {"ai", "assistant"}:  # 如果明确不是AI/助手消息
            return ""  # 跳过用户、人类、系统或工具消息，避免把问题当作答案
        content = message.get("content", "")  # 从字典消息中读取content字段，缺失则为空字符串
    elif isinstance(message, (tuple, list)) and len(message) >= 2:  # 如果消息是二元组或列表格式
        message_type = str(message[0]).lower()  # 读取第一项作为消息类型并统一转为小写
        if message_type and message_type not in {"ai", "assistant"}:  # 如果明确不是AI/助手消息
            return ""  # 跳过非助手消息，避免误提取用户输入
        content = message[1]  # 读取第二项作为消息正文
    else:  # 如果消息是LangChain消息对象或其他对象格式
        message_type = str(getattr(message, "type", "") or getattr(message, "role", "")).lower()  # 读取对象消息类型或角色
        if message_type and message_type not in {"ai", "assistant"}:  # 如果明确不是AI/助手消息
            return ""  # 跳过非助手消息
        content = getattr(message, "content", "")  # 获取消息content属性，缺失则为空字符串
    if isinstance(content, list):  # 如果content是多模态列表
        parts = []  # 创建文本片段列表
        for part in content:  # 遍历多模态内容片段
            if isinstance(part, dict):  # 如果片段是字典
                parts.append(part.get("text", ""))  # 提取text字段加入片段列表
            else:  # 如果片段不是字典
                parts.append(str(part))  # 转成字符串加入片段列表
        content = "".join(parts)  # 拼接多模态文本片段
    return str(content) if content else ""  # 返回文本内容，空内容则返回空字符串
