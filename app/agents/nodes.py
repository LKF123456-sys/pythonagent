...
def _init_metrics() -> None:  # 定义初始化Prometheus指标的私有函数
    global _agent_requests_total, _llm_response_duration  # 声明使用全局变量
    if _agent_requests_total is not None:  # 如果计数器已初始化
        return  # 直接返回，避免重复初始化
    try:  # 开始异常捕获块
        from prometheus_client import Counter, Histogram  # 尝试导入Prometheus客户端的Counter和Histogram
...
async def preprocess_node(state: AgentState) -> dict:  # 定义预处理节点异步函数，返回状态更新字典
    ...
    try:  # 开始异常捕获块
        memories = await store.retrieve_long_term_memories(  # 异步检索长期记忆
            user_question, user_id=None, top_k=get_settings().LONG_TERM_TOP_K  # 传入问题、用户ID为None、检索数量
        )
...
async def _analyze_image(image_path: str, user_question: str) -> str:  # 定义图片分析的私有异步函数
    ...
    try:  # 开始异常捕获块
        response = await ollama.AsyncClient(host=settings.OLLAMA_BASE_URL).chat(  # 异步调用Ollama客户端
...
async def supervisor_node(state: AgentState) -> dict:  # 定义调度主管节点异步函数
    ...
    async def _decide(question: str, history: str) -> str:  # 定义内部决策异步函数
        ...
        if RouteAction.SEARCH in result_text:  # 如果结果包含SEARCH
            return RouteAction.SEARCH  # 返回搜索路由
        if RouteAction.RAG in result_text:  # 如果结果包含RAG
...
async def _search_web(user_question: str) -> str:  # 定义联网搜索的私有异步函数
    ...
    keyword_response = await llm.ainvoke([  # 异步调用LLM提取搜索关键词
    ...
    search_response = tavily_client.search(  # 执行搜索
        query=search_query, search_depth="basic", max_results=3, include_answer=True  # 设置查询、深度、最大结果数、包含AI摘要
    )
...
async def _generate_with_tools(user_message: str) -> str:  # 定义带工具的回答生成私有异步函数
    ...
    first_message = await _stream_full_message(llm_with_tools, messages)  # 流式生成第一轮完整消息

    tool_calls = getattr(first_message, "tool_calls", None)  # 获取消息中的工具调用
    if tool_calls:  # 如果有工具调用
        # 执行工具并注入结果，第二轮生成最终回答
        tool_results = _execute_tool_calls(tool_calls)  # 执行工具调用获取结果
        if tool_results:  # 如果有工具结果
            enhanced_message = f"{user_message}\n\n[工具执行结果]\n{tool_results}"  # 构造增强消息，包含工具结果
            messages = [  # 重新构造消息列表
                SystemMessage(ANSWER_SYSTEM_PROMPT),  # 系统消息
                HumanMessage(content=enhanced_message),  # 人类消息：增强后的消息
            ]
            final_message = await _stream_full_message(llm, messages)  # 流式生成第二轮最终消息
            return final_message.content  # 返回最终消息内容

    return first_message.content  # 无工具调用则直接返回第一轮内容
