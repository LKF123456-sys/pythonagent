...
from app.core.constants import CONTEXT_COMPRESS_THRESHOLD, ROUTE_CACHE_TTL_SECONDS  # 导入上下文压缩阈值常量和路由缓存TTL秒数常量
...
async def supervisor_decide_cached(question: str, history_context: str, decide_fn) -> str:  # 定义带缓存的路由决策异步函数
    ...
    result = await decide_fn(question, history_context)  # 缓存未命中则调用决策函数获取结果
