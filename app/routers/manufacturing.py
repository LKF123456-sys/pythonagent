...
@router.get("/mfg/fault-codes")  # 注册GET路由，查询故障码
...
@router.get("/mfg/equipment")  # 注册GET路由，查询设备参数
...
@router.post("/mfg/upload-image")  # 注册POST路由，上传工业图片
...
@router.post("/mfg/documents/upload")  # 注册POST路由，上传工业文档
...
@router.get("/mfg/documents")  # 注册GET路由，获取工业文档列表
...
@router.delete("/mfg/documents/{filename}", status_code=204)  # 注册DELETE路由，删除工业文档
...
@router.get("/uploads/{filename}")  # 注册GET路由，提供已上传图片访问
